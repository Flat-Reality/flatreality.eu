...
function partnersHref(path = '/') {
  if (partnersHost) return path
  if (localHost) return `/partners${path === '/' ? '' : path}`
  return `https://partners.flatreality.eu${path}`
}
...
function render(){document.body.classList.remove('leaving');const p=location.pathname.toLowerCase().replace(/\/$/,'')||'/';const partnerRoute=partnersHost?p:(p.startsWith('/partners')?(p.slice(9)||'/'):null);let page,seo
  if(partnerRoute!==null){page=partnerRoute==='/'?partners():catalogPage(partnerRoute);seo={title:partnerRoute==='/'?'Flat Reality Partners - Create More. Manage Less.':`${catalogPages[partnerRoute]?.[0]||'FR Partners'} - Flat Reality Partners`,description:catalogPages[partnerRoute]?.[1]||'Creative production services and technologies from Flat Reality Partners.',canonical:`https://partners.flatreality.eu${partnerRoute==='/'?'':partnerRoute}`};document.body.className='theme-partners'}
